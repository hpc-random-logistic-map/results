...
After profiling the MATLAB code, we realized that our critical path is
...
parallelizing at the file level is impossible, we look to parallelize
...
begins with \texttt{generate\_cmdlines.exe}. We start with \texttt{generate\_cmdlines.exe} file which is the set up file for our simulation.
	\texttt{generate\_cmdlines.exe}: This file will take 4 inputs (L,dr,dx,iter). It will generate all the bash script files. 
Bash Script files: Each of these bash script will invoke \texttt{generate\_rand.exe} once which will take in with the same L given in \texttt{generate\_cmdlines.exe}, an r value which is uniformly distributed between 0 and 4 with step size dr and an output file. 	
